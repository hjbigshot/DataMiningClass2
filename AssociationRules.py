...
def get_freqItems():
    ...
    inducer = Orange.associate.AssociationRulesInducer(
        support=0.1,
        store_examples=True)
    for set in inducer.get_itemsets(data):
        line = ''
        for item in set[0]:
            line += str(data.domain[item[0]].name) + \
                '=' + str(data.domain[item[0]][item[1]]) + '\t'
...
def getAssociateRules():
    ...
    rules = Orange.associate.AssociationRulesInducer(
        data,
        support=0.1,
    ...
    for rule in rules:
        ...
        print('%f   %f  %f  %s  ') % (
            rule.support, rule.confidence, rule.lift, rule)
    f.close()
    print(ruledict)
...
def delRedundent():
    ...
    for i in range(size):
        for j in range(size):
            if(i < j):
                if str(ruledict[i][3]).split(' -> ')[0].find(str(ruledict[j][3]).split(' -> ')[0]) != -1 and ruledict[
                        i][2] < ruledict[j][2] and str(ruledict[i][3]).split(' -> ')[1] == str(ruledict[j][3]).split(' -> ')[1]:
                    rmatrix[i][j] = 1
